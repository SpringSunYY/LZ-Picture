...
| **运营大屏**Picture-back-ui 内嵌 | 1. 实时数据大屏 | 注册趋势、上传趋势、下载趋势 |
...
3. **图库模块（picture-picture）**：图片申请、图片分类、图片评论、图片点赞、图片下载、图片详情、用户图片推荐模型、图片标签、图片标签关联、用户搜索记录、空间扩容、空间文件夹、空间信息、空间成员邀请、空间成员、统计信息、用户行为、用户举报、用户浏览记录；
...
6. **系统管理模块（sys_，若依框架）**：参数配置、部门、字典数据、字典类型、定时任务调度、定时任务调度日志、系统访问记录、菜单权限、通知公告、操作日志、岗位、角色、角色部门关联、角色菜单关联、用户信息、用户岗位关联、用户角色关联；
7. **定时任务模块（qrtz_，若依 Quartz）**：Blob 触发器、日历信息、Cron 触发器、已触发的触发器、任务详细信息、悲观锁、暂停的触发器组、调度器状态、简单触发器、同步行锁、触发器详细信息。
...
本章选取荔枝云图库中具有代表性的 **7 个核心功能**进行详细设计说明。每个核心功能给出功能描述、关键流程图、类结构、关键算法/代码片段，便于审查专家快速理解系统的独创性与复杂度。
...
public class AiGenerateStrategyExecutor {
    ...
    @Resource
    private List<AiGenerateStrategyService> aiGenerateStrategyServiceList;  // Spring 自动注入所有策略
...
public class SpaceAuthUtils {
    ...
    private static final int EXPIRE_TIME = 60 * 30;  // 30分钟
...
#### 5.2.9 生成记录表 `ai_generate_log_info`
...
- 一个用户拥有一个积分账户 `po_account_info`，可产生多条充值订单 `po_points_recharge_info` 和消费记录；
- 一个 AI 模型配置 `ai_model_params_info` 可被多次调用，产生多条 `ai_generate_log_info` 记录；
