<template>
  <div class="picture-table">
    <a-card :title="tableName" :bordered="false">
          ...
          <template v-if="column.dataIndex === 'action'">
            <a-space-compact>
              <a-button
                ...
                v-if="checkPermiSingle('picture:upload:detail') && checkSpaceJoin(record.spaceId)"
                >查看
              </a-button>
              <a-dropdown placement="bottomRight" :trigger="['click']">
                ...
                <template #overlay>
                  <a-menu>
                    <a-menu-item
                      v-if="checkSpaceEditor(record.spaceId) && checkUser(record.userId)"
                    >
                      <a-popconfirm
                        title="确定要删除这条记录吗?"
                        ok-text="是"
                    ...
                    <a-menu-item
                      ...
                      v-if="
                        ...
                        checkSpaceEditor(record.spaceId)
                      "
                      >公开
                    </a-menu-item>
                    <a-menu-item
                      @click="handleUpdate(record.pictureId)"
                      v-if="checkPermiSingle('picture:upload') && checkSpaceEditor(record.spaceId)"
                      >修改
                    </a-menu-item>
...
const spaceName = ref(route.query.spaceName as string)
const tableName = spaceName.value + '——团队图片'
...
const getFolderList = () => {
  folderQuery.value.spaceId = spaceId.value
  // 获取文件夹列表
