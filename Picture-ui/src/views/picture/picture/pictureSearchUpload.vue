<template>
  <div class="picture-search-upload">
    <div class="header">
    ...
    <div class="search-section">
      ...
      <div class="search-bar">
        <input
          ...
          class="search-input"
        />
        <button @click="searchImages" :disabled="loading" class="search-btn">
          {{ loading ? '搜索中...' : '搜索图片' }}
        </button>
      </div>
...
const searchQuery = ref<PictureApiSearchRequest>({
  api: '1',
  model: '1',
  keyword: '',
  currentPage: 1,
...
const searchImages = async (): Promise<void> => {
  if (searchQuery.value.keyword === '') return
