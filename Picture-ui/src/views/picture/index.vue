    ...
    <div class="masonry">
      ...
      >
        <img
          ...
          loading="lazy"
          class="masonry-image"
          :style="{ aspectRatio: `${item.picWidth} / ${item.picHeight}` }"
        />
      </div>
    </div>
...
// 生成静态数据
function generatePictureData(start = 0, count = 30) {
  const result = []
...
.masonry {
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); /* 自动根据屏幕调整列数 */
  gap: 16px; /* 图片之间的间距 */
}
