import { pictureDownload, pictureDownloadByLog, pictureDownloadByMy } from '@/api/common/file.ts'
...
export async function downloadImageByMy(pictureId: string, fileName?: string) {
  try {
    await pictureDownloadByMy(pictureId).then((blob) => {
      // console.log('blob', blob)
      // 如果是 JSON 类型，说明是错误信息
      //@ts-ignore
      if (blob.type !== 'application/octet-stream') {
