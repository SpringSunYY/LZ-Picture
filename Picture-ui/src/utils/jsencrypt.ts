...
// 必须包含完整的 PEM 格式头尾
const backPublicKey = `MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEAmYqTJwI7NFqGiAlS05dr
...
export function encryptBack(txt: string): string | null {
  try {
...
export function encryptFront(txt: string): string | null {
  try {
...
export function decryptFront(txt: string): string | null {
  try {
    const decryptor = new JSEncrypt()
