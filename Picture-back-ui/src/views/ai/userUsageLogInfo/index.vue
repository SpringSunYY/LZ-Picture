    ...
    <el-table v-loading="loading" :data="userUsageLogInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="记录编号" align="center" prop="logId" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="模型编号" align="center" prop="modelId" v-if="columns[2].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="输入参数" align="center" prop="inputParams" v-if="columns[3].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="返回结果" align="center" prop="outputResult" v-if="columns[4].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="请求时间" align="center" prop="requestTime" width="180" v-if="columns[5].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.requestTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="请求时长" align="center" prop="requestDuration" v-if="columns[6].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="消耗Tokens数量" align="center" prop="tokensUsed" v-if="columns[7].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="消耗积分" align="center" prop="pointsUsed" v-if="columns[8].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="目标编号" align="center" prop="targetId" v-if="columns[10].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="模型返回码" align="center" prop="aiStatusCode" v-if="columns[12].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="失败原因" align="center" prop="failReason" v-if="columns[13].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="用户IP地址" align="center" prop="ipAddr" v-if="columns[14].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="创建时间" align="center" prop="createTime" width="180" v-if="columns[19].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.createTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="更新时间" align="center" prop="updateTime" width="180" v-if="columns[20].visible" :show-overflow-tooltip="true">
                <template #default="scope">
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['ai:userUsageLogInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['ai:userUsageLogInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="UserUsageLogInfo">
  import { listUserUsageLogInfo, getUserUsageLogInfo, delUserUsageLogInfo, addUserUsageLogInfo, updateUserUsageLogInfo } from "@/api/ai/userUsageLogInfo";

  const { proxy } = getCurrentInstance();
      const { common_delete, po_points_usage_type, ai_log_usage_type } = proxy.useDict('common_delete', 'po_points_usage_type', 'ai_log_usage_type');
