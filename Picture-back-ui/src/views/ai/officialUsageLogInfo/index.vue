    ...
    <el-table v-loading="loading" :data="officialUsageLogInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="记录编号" align="center" prop="logId" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="管理员编号" align="center" prop="userId" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="输入参数" align="center" prop="inputParams" v-if="columns[4].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="模型返回结果" align="center" prop="outputResult" v-if="columns[5].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="请求时间" align="center" prop="requestTime" width="180" v-if="columns[6].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.requestTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="请求时长" align="center" prop="requestDuration" v-if="columns[7].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="消耗Tokens数量" align="center" prop="tokensUsed" v-if="columns[8].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="失败原因" align="center" prop="failReason" v-if="columns[11].visible" :show-overflow-tooltip="true"/>
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['ai:officialUsageLogInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['ai:officialUsageLogInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="OfficialUsageLogInfo">
  import { listOfficialUsageLogInfo, getOfficialUsageLogInfo, delOfficialUsageLogInfo, addOfficialUsageLogInfo, updateOfficialUsageLogInfo } from "@/api/ai/officialUsageLogInfo";

  const { proxy } = getCurrentInstance();
      const { ai_official_usage_type, common_delete, ai_log_status } = proxy.useDict('ai_official_usage_type', 'common_delete', 'ai_log_status');
