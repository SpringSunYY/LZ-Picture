  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="searchLogInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="搜索记录编号" align="center" prop="searchId" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="用户编号" align="center" prop="userId" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="搜索关键词" align="center" prop="keyword" v-if="columns[2].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="失败原因" align="center" prop="failReason" v-if="columns[6].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="返回数量" align="center" prop="resultCount" v-if="columns[7].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="搜索时间" align="center" prop="createTime" width="180" v-if="columns[8].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.createTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="搜索时长" align="center" prop="searchDuration" v-if="columns[9].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="设备唯一标识" align="center" prop="deviceId" v-if="columns[10].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="浏览器类型" align="center" prop="browser" v-if="columns[11].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="操作系统" align="center" prop="os" v-if="columns[12].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="平台" align="center" prop="platform" v-if="columns[13].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="IP属地" align="center" prop="ipAddress" v-if="columns[14].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="点击次数" align="center" prop="clickCount" v-if="columns[15].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="收藏数" align="center" prop="favoriteCount" v-if="columns[16].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="下载数" align="center" prop="downloadCount" v-if="columns[17].visible" :show-overflow-tooltip="true"/>
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['picture:searchLogInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['picture:searchLogInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="searchLogInfoRef" :model="form" :rules="rules" label-width="80px">
                        ...
                        <el-form-item label="搜索来源" prop="referSource">
                          <el-select v-model="form.referSource" placeholder="请选择搜索来源">
                            <el-option
                                v-for="dict in p_search_refer_source"
                                :key="dict.value"
                                :label="dict.label"
                        ...
                        <el-form-item label="返回数量" prop="resultCount">
                          <el-input v-model="form.resultCount" placeholder="请输入返回数量" />
                        </el-form-item>
                        <el-form-item label="搜索时长" prop="searchDuration">
                          <el-input v-model="form.searchDuration" placeholder="请输入搜索时长" />
                        </el-form-item>
                        <el-form-item label="设备唯一标识" prop="deviceId">
                        ...
                        <el-form-item label="收藏数" prop="favoriteCount">
                          <el-input v-model="form.favoriteCount" placeholder="请输入收藏数" />
                        </el-form-item>
                        <el-form-item label="下载数" prop="downloadCount">
                          <el-input v-model="form.downloadCount" placeholder="请输入下载数" />
                        </el-form-item>
      </el-form>
      <template #footer>
...
<script setup name="SearchLogInfo">
  import { listSearchLogInfo, getSearchLogInfo, delSearchLogInfo, addSearchLogInfo, updateSearchLogInfo } from "@/api/picture/searchLogInfo";

  const { proxy } = getCurrentInstance();
      const { p_search_type, p_search_status, p_search_refer_source } = proxy.useDict('p_search_type', 'p_search_status', 'p_search_refer_source');
  ...
  const data = reactive({
    ...
    rules: {
                ...
                { required: true, message: "收藏数不能为空", trigger: "blur" }
                    ...
                    downloadCount: [
                { required: true, message: "下载数不能为空", trigger: "blur" }
              ]
    },
    //表格展示列
    columns: [
              { key: 0, label: '搜索记录编号', visible: true },
                ...
                { key: 6, label: '失败原因', visible: true },
                { key: 7, label: '返回数量', visible: true },
                { key: 8, label: '搜索时间', visible: true },
                { key: 9, label: '搜索时长', visible: true },
                { key: 10, label: '设备唯一标识', visible: true },
                { key: 11, label: '浏览器类型', visible: true },
                { key: 12, label: '操作系统', visible: true },
                { key: 13, label: '平台', visible: true },
                { key: 14, label: 'IP属地', visible: true },
                { key: 15, label: '点击次数', visible: true },
                { key: 16, label: '收藏数', visible: true },
                { key: 17, label: '下载数', visible: true },
      ],
  });
