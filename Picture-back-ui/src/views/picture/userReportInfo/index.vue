  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="userReportInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="举报编号" align="center" prop="reportId" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="目标对象编号" align="center" prop="targetId" v-if="columns[3].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="封面快照" align="center" prop="targetCover" width="100" v-if="columns[4].visible">
                <template #default="scope">
                  <image-preview :src="scope.row.targetCover" :width="50" :height="50"/>
                </template>
              </el-table-column>
                <el-table-column label="举报原因" align="center" prop="reason" v-if="columns[5].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="举报时间" align="center" prop="createTime" width="180" v-if="columns[6].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.createTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="审核状态" align="center" prop="reviewStatus" v-if="columns[7].visible">
                <template #default="scope">
                      <dict-tag :options="p_report_review_status" :value="scope.row.reviewStatus"/>
                </template>
              </el-table-column>
                <el-table-column label="审核信息" align="center" prop="reviewMessage" v-if="columns[8].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="审核人编号" align="center" prop="reviewUserId" v-if="columns[9].visible" :show-overflow-tooltip="true"/>
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['picture:userReportInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['picture:userReportInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="userReportInfoRef" :model="form" :rules="rules" label-width="80px">
                        ...
                        <el-form-item label="封面快照" prop="targetCover">
...
<script setup name="UserReportInfo">
  import { listUserReportInfo, getUserReportInfo, delUserReportInfo, addUserReportInfo, updateUserReportInfo } from "@/api/picture/userReportInfo";

  const { proxy } = getCurrentInstance();
      const { p_report_target_type, p_report_review_status } = proxy.useDict('p_report_target_type', 'p_report_review_status');
  ...
  const data = reactive({
    ...
    //表格展示列
    columns: [
              { key: 0, label: '举报编号', visible: true },
                { key: 1, label: '用户编号', visible: true },
                { key: 2, label: '目标类型', visible: true },
                { key: 3, label: '目标对象编号', visible: true },
                { key: 4, label: '封面快照', visible: true },
                { key: 5, label: '举报原因', visible: true },
                { key: 6, label: '举报时间', visible: true },
                { key: 7, label: '审核状态', visible: true },
                { key: 8, label: '审核信息', visible: true },
                { key: 9, label: '审核人编号', visible: true },
