    ...
    <el-table v-loading="loading" :data="pictureInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="图片编号" align="center" prop="pictureId" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="上传用户编号" align="center" prop="userId" v-if="columns[10].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="编辑时间" align="center" prop="editTime" width="180" v-if="columns[12].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                ...
                <el-table-column label="图片状态" align="center" prop="pictureStatus" v-if="columns[14].visible">
                <template #default="scope">
                      <dict-tag :options="p_picture_status" :value="scope.row.pictureStatus"/>
                </template>
              </el-table-column>
                <el-table-column label="审核状态" align="center" prop="reviewStatus" v-if="columns[15].visible">
                <template #default="scope">
                      <dict-tag :options="p_picture_review_status" :value="scope.row.reviewStatus"/>
                </template>
              </el-table-column>
                <el-table-column label="审核信息" align="center" prop="reviewMessage" v-if="columns[16].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="审核人编号" align="center" prop="reviewUserId" v-if="columns[17].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="审核时间" align="center" prop="reviewTime" width="180" v-if="columns[18].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.reviewTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="缩略图URL" align="center" prop="thumbnailUrl" width="100" v-if="columns[19].visible">
                <template #default="scope">
                  <image-preview :src="scope.row.thumbnailUrl" :width="50" :height="50"/>
                </template>
              </el-table-column>
                <el-table-column label="所属空间编号" align="center" prop="spaceId" v-if="columns[20].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="所属文件夹编号" align="center" prop="folderId" v-if="columns[21].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="图片主色调" align="center" prop="picColor" v-if="columns[22].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="删除时间" align="center" prop="deletedTime" width="180" v-if="columns[24].visible" :show-overflow-tooltip="true">
                <template #default="scope">
...
<script setup name="PictureInfo">
  import { listPictureInfo, getPictureInfo, delPictureInfo, addPictureInfo, updatePictureInfo } from "@/api/picture/pictureInfo";
  ...
  const data = reactive({
    ...
    columns: [
              { key: 0, label: '图片编号', visible: true },
                ...
                { key: 10, label: '上传用户编号', visible: true },
                { key: 11, label: '创建时间', visible: true },
                { key: 12, label: '编辑时间', visible: true },
                { key: 13, label: '更新时间', visible: true },
                { key: 14, label: '图片状态', visible: true },
                { key: 15, label: '审核状态', visible: true },
                { key: 16, label: '审核信息', visible: true },
                { key: 17, label: '审核人编号', visible: true },
                { key: 18, label: '审核时间', visible: true },
                { key: 19, label: '缩略图URL', visible: true },
                { key: 20, label: '所属空间编号', visible: true },
                { key: 21, label: '所属文件夹编号', visible: true },
                { key: 22, label: '图片主色调', visible: true },
                { key: 23, label: '删除', visible: true },
                { key: 24, label: '删除时间', visible: true },
      ],
  });
