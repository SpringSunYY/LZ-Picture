<template>
  <div class="app-container">
    ...
    <el-table ref="tableRef" v-loading="loading" :data="userViewLogInfoList" @selection-change="handleSelectionChange"  @sort-change="customSort">
            <el-table-column type="selection" width="55" align="center"/>
      <el-table-column label="序号" type="index" width="50"/>
      <el-table-column label="记录编号" align="center" prop="viewId" v-if="columns[0].visible"
      ...
      <el-table-column label="目标对象" align="center" prop="targetId" v-if="columns[3].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="目标内容" align="center" prop="targetContent" v-if="columns[4].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="分数" align="center" prop="score" v-if="columns[5].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="图片分类" align="center" prop="categoryId" v-if="columns[6].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="空间" align="center" prop="spaceId" v-if="columns[7].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="图片标签" align="center" prop="tags" v-if="columns[8].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="封面" align="center" prop="targetCover" width="100" v-if="columns[9].visible">
        <template #default="scope">
          <image-preview :src="scope.row.targetCover" :width="50" :height="50"/>
        </template>
      </el-table-column>
      <el-table-column label="查看时间" align="center" prop="createTime" width="180" v-if="columns[10].visible"
                       :show-overflow-tooltip="true">
        <template #default="scope">
          <span>{{ parseTime(scope.row.createTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="是否统计" align="center" prop="hasStatistics" v-if="columns[11].visible">
        <template #default="scope">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['picture:userViewLogInfo:edit']">修改
          </el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['picture:userViewLogInfo:remove']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
import {
  ...
  delUserViewLogInfo,
  addUserViewLogInfo,
  updateUserViewLogInfo
} from "@/api/picture/userViewLogInfo";

const {proxy} = getCurrentInstance();
const {p_view_log_target_type,common_has_statistics} = proxy.useDict('p_view_log_target_type','common_has_statistics');
...
const data = reactive({
  ...
  columns: [
    ...
    {key: 11, label: '是否统计', visible: false},
