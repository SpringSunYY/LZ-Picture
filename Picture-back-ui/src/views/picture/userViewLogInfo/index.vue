<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
                  <el-form-item label="浏览记录编号" prop="viewId">
                    <el-input
                        v-model="queryParams.viewId"
                        placeholder="请输入浏览记录编号"
                        clearable
    ...
    <el-table v-loading="loading" :data="userViewLogInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="浏览记录编号" align="center" prop="viewId" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="图片分类" align="center" prop="categoryId" v-if="columns[4].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="图片标签" align="center" prop="tags" v-if="columns[5].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="封面URL" align="center" prop="targetCover" width="100" v-if="columns[6].visible">
                <template #default="scope">
                  <image-preview :src="scope.row.targetCover" :width="50" :height="50"/>
                </template>
              </el-table-column>
                <el-table-column label="查看时间" align="center" prop="createTime" width="180" v-if="columns[7].visible" :show-overflow-tooltip="true">
                <template #default="scope">
...
<script setup name="UserViewLogInfo">
  ...
  const data = reactive({
    ...
    //表格展示列
    columns: [
              { key: 0, label: '浏览记录编号', visible: true },
                ...
                { key: 6, label: '封面URL', visible: true },
                { key: 7, label: '查看时间', visible: true },
      ],
  });
