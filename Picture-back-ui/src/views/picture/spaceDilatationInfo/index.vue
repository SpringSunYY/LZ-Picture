  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="spaceDilatationInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="申请编号" align="center" prop="dilatationId" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="扩容KEY" align="center" prop="dilatationKey" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="空间编号" align="center" prop="spaceId" v-if="columns[2].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="空间名称" align="center" prop="spaceName" v-if="columns[3].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="扩容单价" align="center" prop="dilatationUnit" v-if="columns[6].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="扩容总数" align="center" prop="dilatationTotal" v-if="columns[7].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="消耗积分" align="center" prop="pointsTotal" v-if="columns[8].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="用户" align="center" prop="userId" v-if="columns[9].visible" :show-overflow-tooltip="true"/>
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['picture:spaceDilatationInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['picture:spaceDilatationInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="spaceDilatationInfoRef" :model="form" :rules="rules" label-width="80px">
                        <el-form-item label="扩容KEY" prop="dilatationKey">
                          <el-input v-model="form.dilatationKey" placeholder="请输入扩容KEY" />
                        </el-form-item>
                        <el-form-item label="空间编号" prop="spaceId">
                        ...
                        <el-form-item label="扩容单价" prop="dilatationUnit">
                          <el-input v-model="form.dilatationUnit" placeholder="请输入扩容单价" />
                        </el-form-item>
                        <el-form-item label="扩容总数" prop="dilatationTotal">
                          <el-input v-model="form.dilatationTotal" placeholder="请输入扩容总数" />
                        </el-form-item>
                        <el-form-item label="消耗积分" prop="pointsTotal">
                          <el-input v-model="form.pointsTotal" placeholder="请输入消耗积分" />
                        </el-form-item>
                        <el-form-item label="用户" prop="userId">
...
<script setup name="SpaceDilatationInfo">
  import { listSpaceDilatationInfo, getSpaceDilatationInfo, delSpaceDilatationInfo, addSpaceDilatationInfo, updateSpaceDilatationInfo } from "@/api/picture/spaceDilatationInfo";
  ...
  const data = reactive({
    ...
    columns: [
              { key: 0, label: '申请编号', visible: true },
                { key: 1, label: '扩容KEY', visible: true },
