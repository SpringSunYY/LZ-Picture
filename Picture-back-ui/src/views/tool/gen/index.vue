    ...
    <el-table v-loading="loading" :data="tableList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" width="330" class-name="small-padding fixed-width">
        <template #default="scope">
          ...
          <el-tooltip content="生成数据" placement="top">
            <el-button link type="primary" icon="Plus" @click="handleGenValue(scope.row)"
                       v-hasPermi="['tool:gen:add']"></el-button>
          </el-tooltip>
...
/** 生成数据 */
function handleGenValue(row) {
  const tableId = row.tableId || this.ids[0]
  const tableName = row.tableName || this.tableNames[0]
  router.push({path: '/tool/gen-add-value/index/' + tableId, query: {tableId: tableId}})
}
