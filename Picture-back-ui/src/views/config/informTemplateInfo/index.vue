  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="informTemplateInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="主键" align="center" prop="templateId" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="模版名称" align="center" prop="templateName" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="语言" align="center" prop="locale" v-if="columns[2].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="内容" align="center" prop="content" v-if="columns[5].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="事例" align="center" prop="example" v-if="columns[6].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="变量列表" align="center" prop="variables" v-if="columns[7].visible" :show-overflow-tooltip="true"/>
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['config:informTemplateInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['config:informTemplateInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="informTemplateInfoRef" :model="form" :rules="rules" label-width="80px">
                        ...
                        <el-form-item label="变量列表" prop="variables">
                          <el-input v-model="form.variables" type="textarea" placeholder="请输入内容" />
                        </el-form-item>
...
<script setup name="InformTemplateInfo">
  import { listInformTemplateInfo, getInformTemplateInfo, delInformTemplateInfo, addInformTemplateInfo, updateInformTemplateInfo } from "@/api/config/informTemplateInfo";

  const { proxy } = getCurrentInstance();
      const { c_template_status, c_template_channel, c_template_type } = proxy.useDict('c_template_status', 'c_template_channel', 'c_template_type');
  ...
  const data = reactive({
    ...
    columns: [
              { key: 0, label: '主键', visible: true },
                ...
                { key: 5, label: '内容', visible: true },
                { key: 6, label: '事例', visible: true },
                { key: 7, label: '变量列表', visible: true },
