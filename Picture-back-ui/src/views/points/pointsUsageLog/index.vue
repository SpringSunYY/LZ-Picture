<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
            ...
            icon="Plus"
            @click="handleAdd"
            v-hasPermi="['points:pointsUsageLog:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
            ...
            :disabled="single"
            @click="handleUpdate"
            v-hasPermi="['points:pointsUsageLog:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
            ...
            :disabled="multiple"
            @click="handleDelete"
            v-hasPermi="['points:pointsUsageLog:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
            ...
            icon="Download"
            @click="handleExport"
            v-hasPermi="['points:pointsUsageLog:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList" :columns="columns"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="pointsUsageLogList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="记录编号" align="center" prop="logId" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="使用类型" align="center" prop="usageType" v-if="columns[2].visible">
                <template #default="scope">
                      <dict-tag :options="po_points_usage_type" :value="scope.row.usageType"/>
                </template>
              </el-table-column>
                <el-table-column label="目标编号" align="center" prop="targetId" v-if="columns[3].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="使用前积分" align="center" prop="pointsBefore" v-if="columns[4].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="消费积分" align="center" prop="pointsUsed" v-if="columns[5].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="使用后积分" align="center" prop="pointsAfter" v-if="columns[6].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="IP地址" align="center" prop="ipAddr" v-if="columns[11].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="备注" align="center" prop="remark" v-if="columns[12].visible" :show-overflow-tooltip="true"/>
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['points:pointsUsageLog:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['points:pointsUsageLog:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="pointsUsageLogRef" :model="form" :rules="rules" label-width="80px">
                        <el-form-item label="用户编号" prop="userId">
...
<script setup name="PointsUsageLog">
  import { listPointsUsageLog, getPointsUsageLog, delPointsUsageLog, addPointsUsageLog, updatePointsUsageLog } from "@/api/points/pointsUsageLog";

  const { proxy } = getCurrentInstance();
      const { common_delete, po_points_usage_type } = proxy.useDict('common_delete', 'po_points_usage_type');

  const pointsUsageLogList = ref([]);
  ...
  const data = reactive({
    ...
    columns: [
              { key: 0, label: '记录编号', visible: true },
                ...
                { key: 2, label: '使用类型', visible: true },
                { key: 3, label: '目标编号', visible: true },
                { key: 4, label: '使用前积分', visible: true },
                { key: 5, label: '消费积分', visible: true },
                { key: 6, label: '使用后积分', visible: true },
                { key: 7, label: '设备唯一标识', visible: true },
                { key: 8, label: '浏览器类型', visible: true },
                { key: 9, label: '操作系统', visible: true },
                { key: 10, label: '平台', visible: true },
                { key: 11, label: 'IP地址', visible: true },
  ...
  function getList() {
    ...
    listPointsUsageLog(queryParams.value).then(response => {
            pointsUsageLogList.value = response.rows;
  ...
  function reset() {
    ...
    proxy.resetForm("pointsUsageLogRef");
  }
  ...
  function handleUpdate(row) {
    ...
    getPointsUsageLog(_logId).then(response => {
  ...
  /** 提交按钮 */
  function submitForm() {
    proxy.$refs["pointsUsageLogRef"].validate(valid => {
      if (valid) {
        if (form.value.logId != null) {
          updatePointsUsageLog(form.value).then(response => {
            proxy.$modal.msgSuccess("修改成功");
            open.value = false;
            getList();
          });
        } else {
          addPointsUsageLog(form.value).then(response => {
  ...
  function handleDelete(row) {
    ...
    proxy.$modal.confirm('是否确认删除积分使用记录编号为"' + _logIds + '"的数据项？').then(function() {
      return delPointsUsageLog(_logIds);
    }).then(() => {
      getList();
  ...
  function handleExport() {
    proxy.download('points/pointsUsageLog/export', {
      ...queryParams.value
    }, `pointsUsageLog_${new Date().getTime()}.xlsx`)
  }
