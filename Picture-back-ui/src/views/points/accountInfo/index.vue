  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="accountInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="账户编号" align="center" prop="accountId" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="用户编号" align="center" prop="userId" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="支付密码" align="center" prop="password" v-if="columns[2].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="加密方式" align="center" prop="salt" v-if="columns[3].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="赚取总积分" align="center" prop="pointsEarned" v-if="columns[4].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="使用总积分" align="center" prop="pointsUsed" v-if="columns[5].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="充值总金额" align="center" prop="rechargeAmount" v-if="columns[6].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="积分余额" align="center" prop="pointsBalance" v-if="columns[8].visible" :show-overflow-tooltip="true"/>
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['points:accountInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['points:accountInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="AccountInfo">
  import { listAccountInfo, getAccountInfo, delAccountInfo, addAccountInfo, updateAccountInfo } from "@/api/points/accountInfo";
  ...
  const data = reactive({
    ...
    columns: [
              { key: 0, label: '账户编号', visible: true },
                ...
                { key: 2, label: '支付密码', visible: true },
                { key: 3, label: '加密方式', visible: true },
