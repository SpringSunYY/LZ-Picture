<template>
  <div class="app-container">
    ...
    <el-table ref="tableRef" v-loading="loading" :data="withdrawalOrderInfoList"
              @selection-change="handleSelectionChange" @sort-change="customSort">
      <el-table-column type="selection" width="55" align="center"/>
      <el-table-column label="序号" type="index" width="50"/>
      <el-table-column label="提现订单编号" align="center" prop="withdrawalId" v-if="columns[0].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="用户编号" align="center" prop="userId" v-if="columns[1].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="提现积分" align="center" prop="pointsWithdrawal" v-if="columns[2].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="提现金额" align="center" prop="amountWithdrawal" v-if="columns[3].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="平台抽成金额" align="center" prop="platformFee" v-if="columns[4].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="用户实际收到金额" align="center" prop="userReceivedAmount" v-if="columns[5].visible"
                       :show-overflow-tooltip="true"/>
      ...
      <el-table-column label="提现账户" align="center" prop="withdrawalAccount" v-if="columns[7].visible"
                       :show-overflow-tooltip="true"/>
      ...
      <el-table-column label="提现平台订单号" align="center" prop="withdrawalPlatformOrder" v-if="columns[9].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="交易编号" align="center" prop="transactionId" v-if="columns[10].visible"
                       :show-overflow-tooltip="true"/>
      ...
      <el-table-column label="审核人编号" align="center" prop="reviewUserId" v-if="columns[13].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="审核建议" align="center" prop="reviewRemark" v-if="columns[14].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="完成时间" align="center" prop="accomplishTime" width="180" v-if="columns[15].visible"
                       :show-overflow-tooltip="true">
        <template #default="scope">
      ...
      <el-table-column label="IP地址" align="center" prop="ipAddr" v-if="columns[20].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="创建时间" align="center" prop="createTime" width="180" v-if="columns[21].visible"
                       :show-overflow-tooltip="true">
        <template #default="scope">
          <span>{{ parseTime(scope.row.createTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="更新时间" align="center" prop="updateTime" width="180" v-if="columns[22].visible"
                       :show-overflow-tooltip="true">
        <template #default="scope">
          <span>{{ parseTime(scope.row.updateTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="提现失败原因" align="center" prop="failReason" v-if="columns[23].visible"
                       :show-overflow-tooltip="true"/>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['points:withdrawalOrderInfo:edit']">修改
          </el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['points:withdrawalOrderInfo:remove']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
import {
  listWithdrawalOrderInfo,
  getWithdrawalOrderInfo,
  delWithdrawalOrderInfo,
  addWithdrawalOrderInfo,
  updateWithdrawalOrderInfo
} from "@/api/points/withdrawalOrderInfo";

const {proxy} = getCurrentInstance();
const {
  common_delete,
  po_approval_status,
  po_withdrawal_status
} = proxy.useDict('common_delete', 'po_approval_status', 'po_withdrawal_status');
