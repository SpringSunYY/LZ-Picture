<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
                  ...
                  <el-form-item label="异常记录时间" style="width: 308px">
                    <el-date-picker
    ...
    <el-table v-loading="loading" :data="errorLogInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="异常编号" align="center" prop="errorId" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="用户编号" align="center" prop="userId" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="支付方式" align="center" prop="methodId" v-if="columns[2].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="异常类型" align="center" prop="errorType" v-if="columns[3].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="异常信息" align="center" prop="errorMessage" v-if="columns[4].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="异常详细信息" align="center" prop="errorDetails" v-if="columns[5].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="关联订单编号" align="center" prop="relatedOrderId" v-if="columns[6].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="异常记录时间" align="center" prop="createTime" width="180" v-if="columns[7].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.createTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="设备唯一标识" align="center" prop="deviceId" v-if="columns[8].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="浏览器类型" align="center" prop="browser" v-if="columns[9].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="操作系统" align="center" prop="os" v-if="columns[10].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="平台" align="center" prop="platform" v-if="columns[11].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="IP地址" align="center" prop="ipAddr" v-if="columns[12].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="解决状态" align="center" prop="resolveStatus" v-if="columns[13].visible">
                <template #default="scope">
                      <dict-tag :options="po_error_resolve_status" :value="scope.row.resolveStatus"/>
                </template>
              </el-table-column>
                <el-table-column label="解决时间" align="center" prop="resolveTime" width="180" v-if="columns[14].visible" :show-overflow-tooltip="true">
                <template #default="scope">
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['points:errorLogInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['points:errorLogInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="errorLogInfoRef" :model="form" :rules="rules" label-width="80px">
                        ...
                        <el-form-item label="异常详细信息" prop="errorDetails">
...
<script setup name="ErrorLogInfo">
  import { listErrorLogInfo, getErrorLogInfo, delErrorLogInfo, addErrorLogInfo, updateErrorLogInfo } from "@/api/points/errorLogInfo";

  const { proxy } = getCurrentInstance();
      const { po_error_resolve_status } = proxy.useDict('po_error_resolve_status');
  ...
  const data = reactive({
    ...
    //表格展示列
    columns: [
              { key: 0, label: '异常编号', visible: true },
                { key: 1, label: '用户编号', visible: true },
                { key: 2, label: '支付方式', visible: true },
                { key: 3, label: '异常类型', visible: true },
                { key: 4, label: '异常信息', visible: true },
                { key: 5, label: '异常详细信息', visible: true },
                { key: 6, label: '关联订单编号', visible: true },
                { key: 7, label: '异常记录时间', visible: true },
                { key: 8, label: '设备唯一标识', visible: true },
                { key: 9, label: '浏览器类型', visible: true },
                { key: 10, label: '操作系统', visible: true },
                { key: 11, label: '平台', visible: true },
                { key: 12, label: 'IP地址', visible: true },
                { key: 13, label: '解决状态', visible: true },
                { key: 14, label: '解决时间', visible: true },
