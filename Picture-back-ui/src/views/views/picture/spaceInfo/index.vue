  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="spaceInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="空间编号" align="center" prop="spaceId" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="空间名称" align="center" prop="spaceName" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="空间封面URL" align="center" prop="spaceAvatar" width="100" v-if="columns[2].visible">
                ...
                <el-table-column label="存储配置" align="center" prop="ossConfig" v-if="columns[4].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="最大容量" align="center" prop="maxSize" v-if="columns[5].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="最大文件数" align="center" prop="maxCount" v-if="columns[6].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="已用容量" align="center" prop="totalSize" v-if="columns[7].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="文件总数" align="center" prop="totalCount" v-if="columns[8].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="所属用户" align="center" prop="userId" v-if="columns[9].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="空间描述" align="center" prop="spaceDesc" v-if="columns[10].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="成员上限" align="center" prop="memberLimit" v-if="columns[13].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="当前成员数" align="center" prop="currentMembers" v-if="columns[14].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="最后上传时间" align="center" prop="lastUpdateTime" width="180" v-if="columns[16].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.lastUpdateTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="最后更新时间" align="center" prop="updateTime" width="180" v-if="columns[17].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                ...
                <el-table-column label="删除时间" align="center" prop="deletedTime" width="180" v-if="columns[19].visible" :show-overflow-tooltip="true">
                <template #default="scope">
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['picture:spaceInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['picture:spaceInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="SpaceInfo">
  ...
  const data = reactive({
    ...
    //表格展示列
    columns: [
              { key: 0, label: '空间编号', visible: true },
                { key: 1, label: '空间名称', visible: true },
                { key: 2, label: '空间封面URL', visible: true },
                { key: 3, label: '存储类型', visible: true },
                { key: 4, label: '存储配置', visible: true },
                { key: 5, label: '最大容量', visible: true },
                { key: 6, label: '最大文件数', visible: true },
                { key: 7, label: '已用容量', visible: true },
                { key: 8, label: '文件总数', visible: true },
                { key: 9, label: '所属用户', visible: true },
                { key: 10, label: '空间描述', visible: true },
                { key: 11, label: '空间状态', visible: true },
                { key: 12, label: '空间类型', visible: true },
                { key: 13, label: '成员上限', visible: true },
                { key: 14, label: '当前成员数', visible: true },
                { key: 15, label: '创建时间', visible: true },
                { key: 16, label: '最后上传时间', visible: true },
                { key: 17, label: '最后更新时间', visible: true },
                { key: 18, label: '删除', visible: true },
                { key: 19, label: '删除时间', visible: true },
      ],
  });
