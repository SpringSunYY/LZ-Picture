...
​	查询任务和生图同理。以实现的即梦AI为例，生图有同步和异步，同步生成成功后直接成功，异步则需要查询任务，当然，后续还需要实现对应的定时任务去查询未完成的任务，当前还没有实现，目前实现的是直接用户查询自己的列表之后马上查询任务。
...
​	本项目设计比较临乱，所有功能都是自己独立开发，且有些功能都是后面才想到的，期间也放弃了很多功能，也是自己当前独立开发的最大一个项目，如果有什么疑问，或者缺点，也欢迎您提出，十分感谢。
